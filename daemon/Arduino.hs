...
controlLoop arduinoPath refMVar pvMVar = withArduino False arduinoPath $ do
    liftIO $ noticeM rootLoggerName "Daemon startup finished"
    let sensor      = analog  0
    ...
    forever $ do
        ...
        liftIO $ debugM "Regulator" $ "Ref: " ++ show reference
                                      ...
                                      ++ " height: " ++ show fillHeight

        liftIO $ debugM "Regulator" $ "Error: " ++ show error ++ " integral: "
                                      ++ show new_integral
...
sensorValueFunc value = fromIntegral value * sensor_constant
                            where sensor_constant | value < 61  = 0
                                                  | value < 80  = 0.0533
                                                  | value < 150 = 0.0431
                                                  | value < 200 = 0.0375
                                                  | value < 220 = 0.0341
